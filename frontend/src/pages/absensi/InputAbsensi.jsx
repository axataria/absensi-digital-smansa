...
export default function InputAbsensi() {
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
        ...
        {siswaList.length > 0 && (
          <button
            ...
            className="flex items-center justify-center gap-2 px-6 py-3 rounded-2xl
                       ...
                       hover:-translate-y-0.5 transition-all disabled:opacity-40 disabled:cursor-not-allowed shrink-0"
          >
      ...
      {!kelasId ? (
        <div className="bg-white rounded-3xl p-16 text-center border border-slate-200 shadow-2xs">
            ...
            <table className="w-full text-sm text-slate-800">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  <th className="text-left px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-400 w-16">No</th>
                  <th className="text-left px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-400">Data Siswa</th>
                  <th className="text-right px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-400">Status Kehadiran</th>
                </tr>
              </thead>
                ...
                {siswaList.map((siswa, index) => {
                  ...
                  return (
                    <tr key={siswa.siswa_id} className="hover:bg-slate-50/80 transition-colors">
                      <td className="px-6 py-4 text-xs font-bold text-slate-400 align-middle">{index + 1}</td>
                      ...
                      <td className="px-6 py-4 align-middle">
                        <div className="flex items-center gap-3.5">
                          ...
                          <div className="flex flex-col gap-0.5">
                            <p className="text-sm font-bold text-slate-900 leading-snug">{siswa.nama_lengkap}</p>
                            <p className="text-xs font-medium text-slate-400">NIS: {siswa.nis}</p>
                          </div>
                        </div>
                      </td>
